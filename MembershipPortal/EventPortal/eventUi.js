...
function renderClasses(classes) {
    ...
    classes.forEach(cls => {
        ...
        renderClass(cls, classContainer);
    });
}
 ...
 */
function renderClass(cls, container) {
    if (!container) {
    ...
    if (eventDate < now) {
        ...
        msg.style.textAlign = 'center';
        msg.style.margin = '16px 0';
        msg.innerHTML = `Please <a href="${sharedConfig.baseUrl}?view=member&eventId=${cls.id}" target="_blank" style="color: #3367d6;">Provide an Email</a> to sign up for this class.`;
        card.appendChild(msg);
    }

    if (sharedConfig.levels[currentMember.registration?.level].value >= sharedConfig.levels.Administrator.value) {
...
*/
function createImageUploadField(cls, form) {
...
function editEvent(cls) {
    ...
    const cancelBtn = createButton('Cancel', () => {
        ...
        loadEvents(); // Reload the events (or return to the previous view)
    });
...
document.addEventListener('DOMContentLoaded', () => {
    showSpinner();
    document.getElementById('memberName').textContent = `${currentMember.firstName} ${currentMember.lastName}`;
